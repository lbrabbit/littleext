...
function findcredit(buffer){
  var tmp = buffer.lastIndexOf(mode.br+icocredit);
  if (tmp<0) {
    if (mode.br!="\n") {
      tmp = buffer.lastIndexOf("\n"+icocredit);
    }
    if (tmp<0) {
      tmp = buffer.lastIndexOf(icocredit);
    }    
  }
  return tmp;
}
function addcredit(formText,instring,id) {
  ...
  if (tmp>=0) {
    var tmp1=buffer.lastIndexOf(bannerEnd);
    if (tmp1>=0) {
      bodyEnd=buffer.substring(tmp1+bannerEnd.length);
    }    
    buffer = buffer.substring(0,tmp);
  }
  bodyLen=buffer.length+instring.length;
  var extra = instring+mode.br+icocredit;
  
  if (creditID.indexOf(id)<0) {
    creditID.push(id);
  }
  for (var i=0;i<creditID.length;i++) {
    extra+=mode.br+addcreditbanner(creditID[i]);
  }  
  extra+=mode.br+mode.br+addcreditbanner('hostbanner');
  extra+=bannerEnd+bodyEnd;
  if (formText.tagName=='BODY'){
    formText.innerHTML=buffer+extra;
  }else {
    formText.value=buffer+extra;
    formText.scrollTop=formText.scrollHeight
      *buffer.length/(buffer.length+extra.length);
